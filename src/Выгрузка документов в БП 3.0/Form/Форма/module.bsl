...
Процедура ПриОткрытии()
	...
	ЗаполнитьСписокТиповДоспутныхДляУдаления(УдаляемыеДанные);
...
Процедура ВыполнитьВыгрузкуИзФормы()
	...
	Если НЕ НепосредственноеЧтениеВИБПриемнике Тогда
		...
		Если Врег(СокрЛП(ИмяФайлаПравилОбмена)) = Врег(СокрЛП(ИмяФайлаОбмена)) Тогда
			...
			Предупреждение("Невозможно выполнить выгрузку! Файл правил обмена не может совпадать с файлом данных. Выберите другой файл для выгрузки данных.");
			Возврат;
...
Процедура УдалитьНажатие(Элемент)
	...
	ОтветПользователя = Вопрос("Удалить выбранные данные в информационной базе?", РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет);
...
Процедура ВосстановитьПростойОтбор(СтрокаТаблицыСОтбором)
	...
	СтрокаОсновныхПВД = ТаблицаПравилВыгрузки.Строки.Найти(СтрокаТаблицыСОтбором.Наименование, "Наименование", Истина);
...
Процедура ИмяФайлаПравилПриИзменении(Элемент)
	...
	ОтветПользователя = Вопрос("Загрузить правила обмена данными?", РежимДиалогаВопрос.ДаНет, 15, КодВозвратаДиалога.Да);
...
Процедура ИмяФайлаДанныхПриИзменении(Элемент)
	...
	Если Не ПустаяСтрока(ИмяФайлаПравилОбмена)
		...
		Предупреждение("Файл правил обмена не может совпадать с файлом данных. Выберите другой файл для выгрузки данных.");
...
Процедура ДействияФормыЗагрузитьПравила(Кнопка)
	...
	ИмяФайлаПравилОбмена = ИмяВременногоФайлаПравилОбмена;
	ЗагрузитьПравилаОбмена();
	
		
	//Для Каждого Стр Из ТаблицаНастройкиПараметров Цикл 
	//	
	//	Если ТипЗнч(Стр.Значение) = Тип("Булево") Тогда 
	//		
	//		Если Стр.Наименование = "ВыгружатьНеизменившиесяДокументы" 	ИЛИ
	//			 Стр.Наименование = "ДобавлятьКомментарийПриЗагрузке" 	ИЛИ
	//			 Стр.Наименование = "ВыгружатьАналитикуПоСкладам" 		Тогда 
	//			 
	//				Стр.Значение = Истина;		
	//		КонецЕсли;              		
	//		
	//	ИначеЕсли ТипЗнч(Стр.Значение) = Тип("СправочникСсылка.Организации") Тогда 
	//		Стр.Значение = Справочники.Организации.НайтиПоКоду("П00000025");
	//	КонецЕсли;
	//	
	//КонецЦикла;
